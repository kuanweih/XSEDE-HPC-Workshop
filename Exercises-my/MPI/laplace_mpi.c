...
int main(int argc, char *argv[]) {
    ...
    if ( npes != NPES ){
        if ( my_PE_num == 0 ){
            printf( "Wrong number of PEs: %d != %d. \n", npes, NPES );
        } // if
    ...
    // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    if ( my_PE_num == 0 ){
        printf( "Tell me the maximum iterations [100-4000]?\n" );
    ...
    while ( dt_global > MAX_TEMP_ERROR && iteration <= max_iterations ) {
        ...
        // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        // TODO define array_receive and array_send
...
void initialize(int npes, int my_PE_num){
    ...
    tMin = (my_PE_num)*TMAX/npes;
    tMax = (my_PE_num+1)*TMAX/npes;
    ...
    if (my_PE_num == npes-1)
        for (j=0; j<=COLUMNS+1; j++)
            Temperature_last[ROWS+1][j] = (TMAX/COLUMNS) * j;
}
